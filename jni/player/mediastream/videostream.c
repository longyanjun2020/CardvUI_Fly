...
static void video_display(player_stat_t *is)
{
    ...
    if (is->decode_type == SOFT_DECODING /*&& AV_PIX_FMT_YUV420P != is->p_vcodec_ctx->pix_fmt*/) {
...
static int open_video_playing(void *arg)
{
    ...
    is->p_frm_buffer = (uint8_t *)av_malloc(buf_size);
    if (is->p_frm_buffer == NULL)
    {
    ...
    printf("convert[%p] fmt[%d] res[%dx%d]\n", is->img_convert_ctx,
        is->p_vcodec_ctx->pix_fmt, is->p_vcodec_ctx->width, is->p_vcodec_ctx->height);
...
static int open_video_stream(player_stat_t *is)
{
    ...
    switch (p_codec_par->codec_id)
    {
        case AV_CODEC_ID_H264 :
            p_codec = avcodec_find_decoder_by_name("h264");//sshevc
            is->decode_type = SOFT_DECODING;
            break;

        case AV_CODEC_ID_HEVC :
            p_codec = avcodec_find_decoder_by_name("hevc");//sshevc
            is->decode_type = SOFT_DECODING;//HARD_DECODING;
            break;
    ...
    printf("open codec: %s:%d\n",p_codec->name,is->decode_type);
